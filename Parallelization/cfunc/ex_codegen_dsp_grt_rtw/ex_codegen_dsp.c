...
static void ex_codegen_dsp_output(void)
{
  ...
  RandSrc_GZ_D(&ex_codegen_dsp_B.RandomSource,
               &ex_codegen_dsp_P.RandomSource_MeanVal, 1,
  ...
  j = 0;
  while (j < 1) {
    acc1 += ex_codegen_dsp_B.RandomSource *
      ex_codegen_dsp_P.GeneratedFilterBlock_Coefficien[0];
    j = 1;
  }

  for (j = 0; j < 31; j++) {
    acc1 += ex_codegen_dsp_P.GeneratedFilterBlock_Coefficien[1 + j] *
      ex_codegen_dsp_DW.GeneratedFilterBlock_states[j];
  }

  ex_codegen_dsp_B.GeneratedFilterBlock = acc1;

  /* Update delay line for next frame */
  for (j = 29; j >= 0; j--) {
    ex_codegen_dsp_DW.GeneratedFilterBlock_states[1 + j] =
      ex_codegen_dsp_DW.GeneratedFilterBlock_states[j];
  }

  ex_codegen_dsp_DW.GeneratedFilterBlock_states[0] =
  ...
  rt_UpdateLogVar((LogVar *)(LogVar*)
                  ...
                  &ex_codegen_dsp_B.LMSFilter_o3[0], 0);
}
...
RT_MODEL_ex_codegen_dsp_T *ex_codegen_dsp(void)
{
  ...
  /* block I/O */
  ex_codegen_dsp_M->blockIO = ((void *) &ex_codegen_dsp_B);
